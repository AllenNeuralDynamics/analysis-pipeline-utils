...
from analysis_pipeline_utils.utils_analysis_dispatch import get_data_asset_paths_from_query, get_s3_input_information, get_input_model_list
...
def test_get_data_asset_ids_from_query(mock_docdb_client):
    ...
    mock_docdb_client.aggregate_docdb_records.return_value = [
        {"_id": "group1", "asset_location": ["id1/path/to/bucket", "id2/path/to/bucket"]},
...
def test_get_s3_input_information_split_true(mock_s3fs):
    ...
    mock_s3fs.glob.return_value = [
        "bucket/key/file1.tif", "bucket/key/file2.tif"
    ]
...
@patch('analysis_pipeline_utils.utils_analysis_dispatch.get_s3_input_information')
def test_flat_input_no_parameters(mock_get_s3_info):
    ...
    result = get_input_model_list(input_paths, file_extension=".tif", split_files=True)
...
def test_flat_input_with_parameters(mock_get_s3_info):
    ...
    result = get_input_model_list(input_paths, file_extension=".tif", split_files=True, distributed_analysis_parameters=parameters)
...
def test_nested_input_no_parameters(mock_get_s3_info):
    ...
    result = get_input_model_list(input_paths, file_extension=".tif", split_files=False)

    assert len(result) == 1  # One group
    assert result[0].s3_location == ["s3://bucket1", "s3://bucket2"]
    assert result[0].file_location == ["s3://bucket1/file1.tif", "s3://bucket1/file2.tif"]
...
def test_nested_input_with_parameters(mock_get_s3_info):
    ...
    input_paths = [["s3://bucket1", "s3://bucket2"]]  # one group of assets
    result = get_input_model_list(input_paths, file_extension=".tif", split_files=False, distributed_analysis_parameters=params)
